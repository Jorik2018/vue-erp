...
function createAxiosProxy(baseUrl: string) {
    return async function proxy(config: any): Promise<[number, any]> {
        const url = new URL(
...
export async function setupAxiosMock() {
    ...
    //mock.onAny(/^\/api\/file(\/.*)?$/).reply(createAxiosProxy(import.meta.env.VITE_BASE_URL_GRA))
    ...
    mock.onGet(/\/api\/hr\/employee\/\d+\/\d+/).reply(200, {
        "data": [
            {
                "n": "1",
    ...
    mock.onAny('/api/auth/validate').reply(config => {
        return [200, { id: 12 }]
    });

    /*mock.onAny(/^\/api\/auth(\/.*)?$/).reply(config => axios({
        ...config,
        url: new URL(config.url.replace(/^\/api\/auth/, ''), import.meta.env.VITE_BASE_URL_AUTH_API).toString(),
        headers: {
            ...config.headers,
            Authorization: `Bearer ${import.meta.env.VITE_JWT_GRA}`
        }
    }))
*/
    // fallback
    ///mock.onAny(/^\/api\/.*/).reply(config => {
       /* console.log('FALLBACK MATCH', config.url)

        return axios({
            ...config,
            url: 'http://localhost:1400' + config.url.replace(/^\/api/, '')
        })
    })*/
return;
    mock.onAny(/^\/api\/.*/).reply(config => {

        //const newUrl = 'http://localhost:1400' + config.url.replace(/^\/api/, '')
        return axios({
            ...config,
            //url: newUrl
        })
