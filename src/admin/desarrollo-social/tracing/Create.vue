<template>
    <v-form v-bind:header="(o.id ? 'Editar ' : 'Crear ') + 'Seguimiento Socioeconomico'" storage="tracing"
        action="/api/desarrollo-social/tracing">
        <div class="v-form"> <label>Provincia:</label> <v-select v-model="o.province" name="province"
                storage="province_selected" required="required"
                v-on:input="$refs.districtSelect.load({ provinceId: parseInt(o.province) })">
                <option value="">Select One...</option>
                <v-options url="/api/directory/province/0/0?regionId=2" storage="province" value-field="code"
                    display-field="name"></v-options>
            </v-select> <label>Distrito:</label> <v-select ref="districtSelect" v-bind:autoload="false"
                storage="district_selected" v-bind:disabled="!o.province" v-model="o.district" name="district"
                required="required" v-on:input="$refs.localitySelect.load({ district: o.district })">
                <option value="">Select One...</option>
                <v-options url="/api/directory/district/0/0" storage="district" value-field="code"
                    display-field="name"></v-options>
            </v-select> <label>Poblado:</label> <v-select ref="localitySelect" v-bind:autoload="false"
                v-bind:disabled="!o.district" v-model="o.locality">
                <option value="">Select One...</option>
                <v-options url="/api/directory/town/0/0" value-field="id" storage="town"
                    display-field="name"></v-options>
            </v-select> <label>Direcci&oacute;n:</label> <v-textarea type="text" rows="1" maxlength="100"
                v-model="o.address" title="Direcci&oacute;n"></v-textarea> <label>C&oacute;digo Casa:</label>
            <input type="text" v-model="o.houseCode" maxlength="6" title="C&oacute;digo Casa" /> <label>Condici&oacute;n
                Vivienda:</label> <v-radio-group required="true" v-model="o.houseCondition">
            ...
            &lt;/v-radio-group--> <label>Programas Sociales:</label> <v-checkbox-group v-model="o.socialPrograms">
                ...
                </v-checkbox-group> <label>Programas Municipales:</label> <v-checkbox-group
                    v-model="o.municipalPrograms">
            ...
            &lt;/v-checkbox-group--> <label>Regi&oacute;n de Origen:</label> <v-select v-model="o.originRegion"
                    required="required">
                    <option value="">Select One...</option>
                    <v-options url="/api/directory/region/0/0" storage="region" value-field="code"
                        display-field="name"></v-options>
                </v-select> <label>Condici&oacute;n de Vivienda:</label> <v-radio-group required="true"
                    v-model="o.housingCondition">
                    <v-radio v-for="(item, i) in housingCondition" v-bind:key="i" v-bind:label="item"
                        v-bind:value="'' + (i + 1)"></v-radio>
                </v-radio-group> <label>Agua:</label> <v-radio-group required="true" v-model="o.water">
                    <v-radio v-for="(item, i) in water" v-bind:label="item" v-bind:key="i"
                        v-bind:value="'' + (i + 1)"></v-radio>
                </v-radio-group> <label>Tipo de SS.HH:</label> <v-radio-group required="true" v-model="o.sshhType">
                    <v-radio v-for="(item, i) in sshhType" v-bind:label="item" v-bind:key="i"
                        v-bind:value="'' + (i + 1)"></v-radio>
                </v-radio-group> <label>Alumbrado Electrico:</label> <v-radio-group required="true"
                    v-model="o.electricalLighting">
                    <v-radio v-for="(item, i) in electricalLighting" v-bind:label="item" v-bind:key="i"
                        v-bind:value="'' + (i + 1)"></v-radio>
                </v-radio-group> <label>Conectividad:</label> <v-radio-group required="true" v-model="o.connectivity">
                    <v-radio label="SI" value="1"></v-radio>
                    <v-radio label="NO" value="0"></v-radio>
                </v-radio-group>
                <template v-if="o.connectivity == 1"> <label>Operadores:</label> <v-checkbox-group
                        v-model="o.phoneOperator">
