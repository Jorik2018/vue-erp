<template>
    <v-form id="DsPeopleCreateForm" action="/api/desarrollo-social/census"
        ...
        <div class="v-form" v-if="o"> <label>Familia:</label>
            ...
            <template v-if="!o.ext.bossId"> <label>Regi&amp;oacute;n:</label> <v-select v-model="o.region"
                    storage="region_selected" v-bind:required="true"
                    v-on:input="$refs.province.load({ regionId: o.region })">
                ...
                </v-select> <label>Provincia:</label> <v-select ref="province" storage="province_selected"
                    v-model="o.province" v-bind:autoload="false" v-bind:disabled="!o.region" v-bind:required="true"
                    v-on:input="$refs.district.load({ provinceId: o.province })">
                    <option>Select One...</option>
                ...
                </v-select> <label>Distrito:</label> <v-select ref="district" v-model="o.district"
                    v-bind:autoload="false" v-bind:disabled="!o.province" v-bind:required="true"
                    v-on:input="$refs.ccpp.load({ district: o.district }); $refs.establishment.load({ location: o.district });">
                    <option>Select One...</option>
                ...
                </v-select> <label v-bind:title="o.ccpp">Localidad:</label> <v-select ref="ccpp" v-model="o.ccpp"
                    v-bind:autoload="false" storage="ccpp" v-bind:disabled="!o.district" v-bind:required="true">
                    <option>Select One...</option>
            ...
            <input type="tel" required="required" v-model="o.code" class="numeric center"
                style="text-align:center !important;" maxlength="12" pattern="[0-9]*" /> <label>ApPaterno:</label>
            <input type="text" v-model="o.apPaterno" required="required" maxlength="45" title="ApPaterno" />
            <label>ApMaterno:</label>
            <input type="text" v-model="o.apMaterno" required="required" maxlength="45" title="ApMaterno" />
            <label>Nombres:</label>
            <input type="text" v-model="o.names" required="required" maxlength="45" title="Nombres" />
            <label>Telefono:</label>
            <input type="text" v-model="o.phone" maxlength="12" /> <label>Fecha Nacimiento:</label> <v-calendar
                v-model="o.birthdate" required="required" title="Fecha Nacimiento"></v-calendar>
            <div id="dateDiff" v-if="dateDiff"
                style="    border: 1px solid #ffcb60;     background-color: #f9ff70;     padding: 5px;     margin-top: 5px;">
                HACE {{ dateDiff }}
            </div> <label>Sexo:</label> <v-radio-group required="true" v-model="o.sex">
                <v-radio label="Masculino" value="M"></v-radio>
                <v-radio label="Femenino" value="F"></v-radio>
            </v-radio-group> <label v-bind:title="o.ext.bossId">PARENTESCO con JEFE FAMILIA:</label> <v-radio-group
                required="true" v-model="o.relationship"> <!--v-radio value="1" label="SOCIO"/-->
                ...
                <option v-for="(civilStatus, key) in options.civilStatus" v-bind:key="key" v-bind:value="civilStatus">
                    {{ civilStatus }}
                </option>
            </v-select> <label>Grado de Instrucci&oacute;n:</label> <v-select v-model="o.degreeInstruction"
                required="required">
                <option value="">Select One...</option>
            ...
            <v-select v-model="o.occupation" required="required">
                ...
                <option value="">Select One...</option>
                <option v-for="(assured, key) in options.assured" v-bind:key="key" v-bind:value="key + 1">{{ assured }}
                </option>
            ...
            <v-checkbox v-model="o.hypertensive" label="Hipertenso"></v-checkbox> <label>Intervenci&oacute;n
                COVID?:</label> <v-switch required="true" v-model="o.covid"></v-switch> <v-fieldset
                v-if="(o.covid + '') == 'true'" legend="Ultima Intervencion">
                ...
                <template v-if="o.reactive == 'SI'"> <label>En Aislamiento?:</label> <v-switch required="true"
                        v-model="o.isolation"></v-switch> <v-fieldset v-if="o.isolation">
            ...
            </v-fieldset> <label>Establecimiento:</label> <v-select ref="establishment" v-model="o.establishment"
                v-bind:autoload="false" v-bind:disabled="!o.district" v-bind:required="true">
                <option>Select One...</option>
            ...
            </v-select> <label>Observaci&oacute;n:</label> <v-textarea v-model="o.remark" maxlength="355"
                placeholder="Informacion adicional por ejemplo: localidad, establecimiento, etc."></v-textarea>
            <template> <v-fieldset legend="Coordenadas">
                    ...
                    <div class="right" style="margin-top:10px"> <v-button icon="fa-compass"
                            value="Obtener coordenadas actuales" v-on:click.prevent="getCoordinates"></v-button>
                    </div>
                </v-fieldset>
            </template>
        </div>
...
export default ui({
    ...
    computed: {
        dateDiff() {
            let r = '', f = this.o.birthdate;
            if (f) {
                const r2 = Vue.dateDiff(f, new Date());
                if (r2.ans)
                    r += (r2.ans + ' ANIOS ');
                if (r2.meses)
                    r += (r2.meses + ' MESES ');
    ...
    methods: {
        changeRoute() {
            const me = this, id = me.id, action = me.action;
            if (id) {
                if (action == 'add') {
                    const v = id.split('.');
                    const o = { lat: null, lon: null, bossId: v[1], district: null, masterId: v[0], ext: { bossId: v[1] ? v[1] : null } };
