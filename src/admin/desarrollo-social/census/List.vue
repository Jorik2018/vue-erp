<template>
  <v-form id="DsPeopleList" header="Empadronamiento" action="/admin/desarrollo-social/census">
    <v-table v-bind:selectable="true" v-bind:scrollable="true" rowKey="id" v-bind:pagination="20"
      row-style-class="row.tmpSynchronized?'green':(row.tmpId>0?'yellow':'')" storage="census"
      src="/api/desarrollo-social/census" v-bind:filters="filters">
      <template v-slot:header="">
        <!--v-button value="Crear" icon="fa-plus" class="on" v-on:click.prevent="create"/-->
        <v-button value="Editar" icon="fa-pen" v-if="canRegister()" v-on:click.prevent="edit" v-bind:disabled="rowSelectedCount == 0 || !canRegister(getSelected()[0])
          "></v-button>
        ...
        <v-button icon="fa-search" v-on:click.prevent="view(getSelected())" v-bind:disabled="rowSelectedCount == 0"
          value="Ver"></v-button>
        <v-button value="Eliminar" v-if="canRegister()" icon="fa-trash" v-on:click.prevent="destroy"
          v-bind:disabled="rowSelectedCount == 0"></v-button>
        <v-button value="Descargar" icon="fa-download" v-if="perms.ADMIN_DESARROLLO_SOCIAL || user.uid == 1"
          v-on:click.prevent="download"></v-button>
        ...
        <v-button title="Enviar al Servidor" v-if="online" icon="fa-database" class="on"
          v-bind:disabled="rowSelectedCount == 0 || !canSync(getSelected()[0])" v-on:click.prevent="sync"></v-button>
      </template>
      <template v-slot:default="{ row }">
        ...
        <td width="90" class="center" header="Embarazada">
          <i style="display: inline-block" v-bind:class="row.pregnant ? 'fa fa-check' : ''"></i>
        </td>
        <td width="90" class="center" header="Discapacitado">
          <i style="display: inline-block" v-bind:class="row.disability ? 'fa fa-check' : ''"></i>
        </td>
        <td width="90" class="center" header="Hipertenso">
          <i style="display: inline-block" v-bind:class="row.hypertensive ? 'fa fa-check' : ''"></i>
        </td>
        <td width="80" class="center" header="Puerpera">
          <i style="display: inline-block" v-bind:class="row.puerpera ? 'fa fa-check' : ''"></i>
        </td>
        <td width="80" class="center" header="Intevenci�n COVID?">
          <i style="display: inline-block" v-bind:class="row.covid ? 'fa fa-check' : ''"></i>
        </td>
        <td width="80" class="center" header="Obesidad">
          <i style="display: inline-block" v-bind:class="row.obesity ? 'fa fa-check' : ''"></i>
        </td>
        <td width="80" class="center" header="Diabetes">
          <i style="display: inline-block" v-bind:class="row.diabetes ? 'fa fa-check' : ''"></i>
        </td>
      ...
      <template v-slot:filters>
        <v-filter index="0">
          <input v-model="filters.id" /> </v-filter><v-filter index="1">
          <input v-model="filters.masterId" /> </v-filter><v-filter index="2">
          <input v-model="filters.bossId" /> </v-filter><v-filter index="3">
          <input v-model="filters.province" /> </v-filter><v-filter index="4">
          <input v-model="filters.district" /> </v-filter><v-filter index="5">
          <input v-model="filters.ccpp" /> </v-filter><v-filter index="6">
          <input v-model="filters.code" maxlength="8" /> </v-filter><v-filter index="7">
          <input v-model="filters.apPaterno" /> </v-filter><v-filter index="8">
          <input v-model="filters.apMaterno" /> </v-filter><v-filter index="9">
          <input v-model="filters.names" /> </v-filter><v-filter index="10">
          <input v-model="filters.phone" /> </v-filter><v-filter index="11">
          <input v-model="filters.birthdate" /> </v-filter><v-filter index="12">
          <input v-model="filters.sex" /> </v-filter><v-filter index="13">
          <input v-model="filters.relationship" /> </v-filter><v-filter index="14">
          <input v-model="filters.civilStatus" /> </v-filter><v-filter index="15">
          <input v-model="filters.degreeInstruction" /> </v-filter><v-filter index="16">
          <input v-model="filters.occupation" /> </v-filter><v-filter index="17">
          <input v-model="filters.language" /> </v-filter><v-filter index="18">
          <input v-model="filters.religion" /> </v-filter><v-filter index="19">
          <input v-model="filters.assured" /> </v-filter><v-filter index="20">
          <input v-model="filters.establishment" /> </v-filter><v-filter index="21">
          <input v-model="filters.pregnant" /> </v-filter><v-filter index="22">
          <input v-model="filters.disability" /> </v-filter><v-filter index="23">
          <input v-model="filters.hypertensive" /> </v-filter><v-filter index="24">
          <input v-model="filters.puerpera" /> </v-filter><v-filter index="25">
          <input v-model="filters.covid" /> </v-filter><v-filter index="26">
          <input v-model="filters.obesity" /> </v-filter><v-filter index="27">
          <input v-model="filters.diabetes" /> </v-filter><v-filter index="28">
          <input v-model="filters.lat" /> </v-filter><v-filter index="29">
          <input v-model="filters.user" /> </v-filter><v-filter index="30">
