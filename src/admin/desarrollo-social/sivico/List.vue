    ...
    <ion-content :scroll-events="true">
        <v-form header="Registro MCI" action="/admin/desarrollo-social/sivico">
            <v-table :selectable="true" row-style-class="row.synchronized?'green':(row.tmpId>0?'yellow':'')"
                store="pool" @loaded="ll" :scrollable="true" rowKey="id" :pagination="20" v-on:updated="bindLinks($el)"
                ...
                <template v-slot:header>
                    <v-button value="Enviar" v-if="app.connected" icon="fa-save" :disabled="!rowSelectedCount"
                        v-on:click.prevent="sync"></v-button>
                ...
                <template v-slot="{ row }">
                    ...
                    <td width="120" sorter="row.peoples" class="center" header="Integrantes"> {{ toInt(row.peoples) }}
                    </td>
                    ...
                    <td width="160" class="center" header="Tiempo que Demora en Llegar a E.E.S.S."> {{ row.tiempoAEess }}
                    </td>
                    ...
                    <td width="120" class="center" h-class="hg" header="Proxima Visita"> {{ row.proximaVisita | date }}
                    </td>
                    ...
                    <td width="150" class="center" h-class="hgd" header="Con nacimiento del primer hijo/a"> {{ row.c5 }}
                    </td>
                    <td width="100" class="center" h-class="hgd" header="Con hija/o en edad pre-escolar"> {{ row.c6 }}
                    </td>
                    ...
                    <td width="160" class="center" header="FAMILIOGRAMA">
                        <a target="_new" class="_"
                            :href="row.ext && row.ext.src && row.ext.pending ? row.ext.src : ('/fs/desarrollo-social/sivico/familiograma/' + row.id + '.jpeg')"
                    ...
                    <td width="150" class="center" header="Quincha (ca&ntilde;a de barro), piedra con barro">
                        {{ row.c135 }} </td>
                    ...
                    <td width="200" class="center" h-class="hg"
                        header="DISPOSICION BASURA / Carro recolector (Frecuencia)"> {{ row.c152 }} </td>
                    ...
                    <td width="140" class="center" h-class="hg" header="COMBUSTIBLE COCINA / Le&ntilde;a"> {{ row.c166 }}
                    </td>
                    ...
                    <td width="180" class="center"
                        header="ABASTECIMIENTO AGUA / Red p&uacute;blica dentro de la vivienda"> {{ row.c187 }} </td>
                    ...
                    <td width="140" class="center" h-class="hg" header="Familia cuenta con mochila de emergencia">
                        {{ row.c191 }} </td>
                    <td width="140" class="center" header="Familia cuenta con botiqu&iacute;n de emergencia">
                        {{ row.c192 }} </td>
                    <td width="380" class="center" h-class="hg"
                        header="Su vivienda ¿cuenta con espacios destinados al almacenamiento, conservaci&oacute;n, manipulaci&oacute;n y consumo de alimentos?">
                        {{ row.c193 }} </td>
                    <td width="270" class="center"
                        header="Su cocina ¿cuenta con sistema de eliminaci&oacute;n del humo de cocina o ventilaci&oacute;n?">
                        {{ row.c194 }} </td>
                    ...
                    <td width="250" class="center"
                        header="ANIMALES: De importancia econ&oacute;mica: cabras, cameros, cerdos, vaca, aves de corral">
                        {{ row.c198 }} </td>
                    <td width="100" class="center" h-class="hbd" header="Vacunas"> {{ row.c199 }} </td>
                    <td width="180" class="center" header="Convive con los animales dentro de la vivienda"> {{ row.c200 }}
                    </td>
                    <td width="200" class="center" h-class="hr" header="RIESGOS ENTORNO / Lluvias, inundaciones">
                        {{ row.c202 }} </td>
                    <td width="100" class="center" h-class="hr" header="Basural junto a la vivienda"> {{ row.c203 }} </td>
                    <td width="100" class="center" h-class="hr" header="Inservibles junto a la vivienda"> {{ row.c204 }}
                    </td>
                    <td width="280" class="center" h-class="hr"
                        header="Humos o vapores de productos quimicos de fabr&iacute;cas, industrias o mineria">
                        {{ row.c205 }} </td>
                    <td width="100" class="center" h-class="hr" header="Riesgo de derrumbes, huaycos"> {{ row.c206 }}
                    </td>
                    <td width="140" class="center" h-class="hr" header="Pandillaje, delincuencia"> {{ row.c207 }} </td>
                    <td width="140" class="center" h-class="hr" header="Alcoholismo, drogadicci&oacute;n"> {{ row.c208 }}
                    </td>
                    <td width="100" class="center" h-class="hr" header="Sin alumbrado p&uacute;blico"> {{ row.c209 }}
                    </td>
                    <td width="100" class="center" h-class="hr" header="Pistas no asfaltadas"> {{ row.c210 }} </td>
                    <td width="180" class="center" h-class="hr" header="Vectores (mosquitos, zancudo, roedores, etc)">
                        {{ row.c211 }} </td>
