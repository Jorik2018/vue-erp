  ...
  <v-form header="Gestantes" action="/admin/desarrollo-social/pregnant">
    <v-table :selectable="true" row-style-class="row.synchronized?'green':(row.tmpId>0?'yellow':'')" store="pregnant"
      ...
      <template v-slot:header>
        ...
        <v-button :title="'Crear ' + filters.microredName" value="Crear"
          v-if="1 || can(perms.DESARROLLO_SOCIAL_REGISTER_SIVICO)" icon="fa-plus" class="on"
      ...
      <template v-slot="{ row }">
        ...
        <td width="130" class="center" header="MicroRed">
          <v-filter>
            <div>
              <v-select v-show="filters.red" v-model="filters.microred" ref="microred">
                <option value="">Select One...</option>
                <v-options store="microred" display-field="name" value-field="code" />
              </v-select>
              <input v-show="!filters.red" :title="'microred=' + filters.microredName + '-' + filters"
                v-model="filters.microredName" />
            </div>
          </v-filter>
...
export default ui({
  ...
  data() {
    return {
      t: null, rowSelectedCount2: 0, page: 0, data: [], mode: 0, query: null,
