	...
	<v-form header="CONSULTA COVID" action="/admin/desarrollo-social/covid">
		<div> <v-fieldset legend="BUSQUEDA POR DOCUMENTO IDENTIDAD" class="v-form">
				<v-autocomplete placeholder="Ingrese mas de 5 caracteres y presione ENTER" ref="peopleAutocomplete"
					v-bind:show-selection="true" inputClass="center" v-on:input="inputPeople"
					v-bind:params="{ distinct: 1 }" minQueryLength="5" required="required" v-model="o"
					v-on:complete="peopleComplete" src="/api/desarrollo-social/covid">
					<template v-slot:label="{ selected }">
						<template v-if="selected">
							{{ selected.id }}: {{ selected.apellidoPaterno }} {{ selected.apellidoMaterno }}
							{{ selected.names }}
						</template>
					</template>
					<template v-slot="{ row }">
				...
				<div v-if="!o" class="right" style="margin-top:10px"> <v-button title="Refrescar" icon="fa-sync"
						class="on" v-on:click.prevent="$refs.peopleAutocomplete.search()"></v-button>
				</div>
			</v-fieldset> <v-fieldset v-if="o" legend="Resultados">
				<div v-for="(o, j) in l" v-bind:key="j" class="v-item"
					style="padding: 5px;background-color: black;color:white;line-break: anywhere;">
