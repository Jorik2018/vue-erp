<template>
    <v-form id="DisabledList" header="Discapacitados" action="/admin/desarrollo-social/disabled">
        <v-table v-bind:selectable="true" v-bind:scrollable="true" v-on:synchronized="postSync"
            row-style-class="row.tmpSynchronized?'green':(row.tmpId>0?'yellow':'')" rowKey="id" storage="disabled"
            v-bind:pagination="20" src="/api/desarrollo-social/disabled" v-bind:filters="filters">
            <template v-slot:header=""> <v-button value="Crear" v-if="canRegister()" icon="fa-plus" class="on"
                    v-on:click.prevent="create"></v-button> <v-button value="Editar" v-if="canRegister()" icon="fa-pen"
                    v-on:click.prevent="edit" v-bind:disabled="rowSelectedCount == 0"></v-button> <v-button
                    value="Eliminar" v-if="canRegister()" icon="fa-trash" v-on:click.prevent="destroy"
                    v-bind:disabled="rowSelectedCount == 0"></v-button> <v-button title="Refrescar" icon="fa-sync"
                    v-on:click.prevent="refresh"></v-button> <v-uploader v-on:input="upload"
                    value="Importar"></v-uploader> <v-button title="Enviar al Servidor" v-if="online" icon="fa-database"
                    class="on" v-on:click.prevent="sync"></v-button>
            </template>
            <template v-slot:default="{ row, index }">
                <td class="center" header="#" width="28"> {{ pad(index + 1, 3) }} </td>
                <td width="80" class="center" v-bind:class="{ 'yellow': 0 > row.id }" header="ID">
                    {{ row.id > 0 ? pad(row.id, 6) : '?' }} </td>
                ...
                <td width="130" class="center" header="Cuenta con certificado de discapacidad?">
                    ...
                    <template v-if="row.certified"> <a
                            v-bind:href="'/fs/desarrollo-social/disabled/certified/' + row.id + '.jpeg'" target="_new"> SI
                        </a>
                    </template> <span v-if="row.ext">{{ row.ext.tmpSrc }}</span>
                </td>
                <td width="200" header="Resolución"> {{ row.nroResolution }} </td>
                <td width="200" header="Nro Cerficado"> {{ row.nroCertificate }} </td>
                <td width="80" class="center" header="Fecha Emisión"> {{ row.issuanceDate | date }} </td>
