<template>
  <v-form id="DisabledCreateForm" action="/api/desarrollo-social/disabled"
    v-bind:header="(o.id ? 'Editar ' : 'Crear ') + 'Discapacitado'" storage="disabled">
    <div class="v-form">
      <label>DNI:</label>
      <input v-model="o.code" type="tel" class="numeric center" style="text-align: center !important" maxlength="12"
        pattern="[0-9]*" />
      <label>Nombres:</label>
      ...
      <div id="dateDiff" v-if="dateDiff" style="
      ...
      <v-select v-model="o.province" name="province" storage="province_selected" required="required" v-on:input="
      ...
      <v-select ref="districtSelect" v-bind:autoload="false" storage="district_selected" v-bind:disabled="!o.province"
        v-model="o.district" name="district" required="required"
      ...
      <label>Poblado:</label>
      <v-select ref="localitySelect" v-bind:autoload="false" v-bind:disabled="!o.district" v-model="o.town">
        <option value="">Select One...</option>
        <v-options url="/api/directory/town/0/0" value-field="id" storage="town" display-field="name"></v-options>
      </v-select>
      <label>Dirección:</label>
      <v-textarea type="text" required="true" maxlength="55" v-model="o.address" title="Dirección"></v-textarea>
      <label>Cuenta con certificado de discapacidad?:</label>
      <v-switch required="true" v-model="o.certified"></v-switch>
      <v-fieldset v-if="o.certified + '' == 'true'" legend="Datos del certificado">
        ...
        <div class="right">
          <v-uploader icon="fa-camera" style="margin-top: 10px" value="Obtener Imagen" v-bind:click="uploaderClick"
            v-on:input="changeImage($event)"></v-uploader>
          <a class="_" v-bind:key="'img-' + count" style="display: block">
            <img onerror="this.parentNode.style.display='none'" style="
                width: calc(100% - 4px);
                display: block;
                border: 2px solid green;
                margin-top: 10px;
              " v-bind:src="o.ext.src
                ? o.ext.src
                : baseURL +
      ...
      <v-textarea v-if="o.type == 'Otro' || o.otherType" v-model="o.otherType"></v-textarea>
      ...
      <v-textarea type="text" maxlength="45" v-model="o.devices"
        title="Dispositivos o productos de apoyo que utiliza"></v-textarea>
      ...
      <v-fieldset legend="Persona que asiste" v-if="o.assistance">
        <label>DNI:</label>
        <input v-model="o.assistant" type="tel" class="numeric center" style="text-align: center !important"
        ...
        <v-fieldset legend="Coordenadas">
          ...
          <div class="right" style="margin-top: 10px">
            <v-button icon="fa-compass" value="Obtener coordenadas actuales"
              v-on:click.prevent="getCoordinates"></v-button>
          </div>
        </v-fieldset>
      </template>
    </div>
...
export default ui({
  ...
  methods: {
    ...
    changeRoute() {
      ...
      if (Number(id)) {
        if (0 > id) {
          const d = JSON.parse(localStorage.getItem("disabled"));
