<template>
    ...
    overflow: auto;" class="v-form" :class="o.id < 0 || (o.tmpId && !o.synchronized)
        ? 'yellow'
        : o.tmpId
            ? 'green'
            : ''
        " store="poll" action="/api/poll/main">
        <span class="v-button-top-float" @click="save"><i class="fa fa-save"></i></span>
        <h2 style="
            margin: 0px;
            padding: 12px;
            position: relative;
        ...
        <v-fieldset legend="SECCIÓN DE IDENTIFICACIÓN MUESTRAL Y GEOGRÁFICA DE LOS ENCUESTADOS">
            <label>Departamento:</label>
            <v-select v-model="o.region" @input="$refs.province.load({ code: pad(o.region, 2) })">
                <option value="">Seleccionar Opci&oacute;n</option>
                <v-options store="region" value-field="id" display-field="name" />
            </v-select>
            <label>Provincia:</label>
            <v-select v-model="o.provincia" ref="province" :required="true" :autoload="false" :disabled="!o.region"
                @input="$refs.district.load({ code: o.provincia })">
                <option>Select One...</option>
                <v-options store="province" value-field="code" display-field="name" />
            </v-select>
            <label>Distrito:</label>
            <v-select v-model="o.distrito" ref="district" :autoload="false" store="district_selected"
                @input="$refs.cp.load({ id: o.distrito })" :disabled="!o.provincia" name="district" required="required">
                <option value="">Select One...</option>
                <v-options store="district" value-field="code" display-field="name" />
            </v-select>
            <label>Centro Poblado:</label>
            <v-select ref="cp" v-on:input="changeTown(o.centro_poblado)" autoload="false" :disabled="!o.distrito"
            ...
            <v-button style="margin-top: 10px" icon="fa-map" value="Obtener Geolocalización"
                v-on:click="printCurrentPosition" />
            <div v-if="(o.lat && o.lat != 0) || (o.lon && o.lon != 0) || trayLocation" style="
              margin-top: 10px;
        ...
        <v-fieldset legend="DATOS DEL INFORMANTE">
            ...
            <v-radio-group label="Es jefe/a del hogar" v-model="o.is_boss_house">
                <v-radio value="Si" />
                <v-radio value="No" />
            </v-radio-group>
            <template v-if="o.is_boss_house === 'Si'">
                <v-radio-group label="¿Cuál es relación que mantiene con el jefe/a de hogar?"
                    v-model="o.relation_with_boss_house" :items="relation_with_boss_house" />
            </template>
        </v-fieldset>
        <div class="xyz" v-if="o.id">
            <a :href="'/admin/poll/' + (o.tmpId ? -o.tmpId : o.id) + '/a'"><b>SECCIÓN A: CARACTERÍSTICAS Y SITUACIÓN
                    DE LA
                    VIVIENDA</b> [Responde solo
                el jefe de hogar]</a>
            <a :href="'/admin/poll/' + (o.tmpId ? -o.tmpId : o.id) + '/peoples'"><b>SECCIÓN B: CARACTERÍSTICAS
                    SOCIODEMOGRÁFICAS
                    I</b>
                ...
                <b>SECCIÓN C: FUENTES DE INGRESO</b> [Debe ser respondida por todo miembro del hogar mayor de 14
                años de edad
                ]</a>
            <a :href="'/admin/poll/' + (o.tmpId ? -o.tmpId : o.id) + '/d'"><b>SECCIÓN D: CARACTERÍSTICAS
                    SOCIODEMOGRÁFICAS
                    II</b> [Debe ser respondida
                por el jefe/a de hogar]</a>
            <a :href="'/admin/poll/' + (o.tmpId ? -o.tmpId : o.id) + '/e'"><b>SECCIÓN E: MÓDULO DE EDUCACIÓN</b>
                [Idealmente debe
                ser respondida por
            ...
            <a :href="'/admin/poll/' + (o.tmpId ? -o.tmpId : o.id) + '/f'"><b>SECCIÓN F: MÓDULO DE SALUD</b> [Debe
                ser
                respondida por el jefe/a de
                hogar] - <b>SECCIÓN G: MÓDULO DE SEGURIDAD ALIMENTARIA</b> [Debe ser respondida por
                el jefe/a de hogar]</a>
            <a :href="'/admin/poll/' + (o.tmpId ? -o.tmpId : o.id) + '/h'"><b>SECCIÓN H: OFERTA DE FUERZA DE TRABAJO
                    Y
                    PERCEPCIÓN DE INGRESOS</b> -
                <b>SECCIÓN I. EMPRENDIMIENTO</b>
            </a>
            <a :href="'/admin/poll/' + (o.tmpId ? -o.tmpId : o.id) + '/j'">SECCIÓN J: ACTIVIDADES AGROPECUARIAS</a>
            <a :href="'/admin/poll/' + (o.tmpId ? -o.tmpId : o.id) + '/k'">SECCIÓN K: REDES SOCIALES Y MOVILIDAD EN
                EL
                TERRITORIO</a>
        </div>
    </v-form>
...
export default ui({
    ...
    data() {
        return {
            ...
            districtList: [],
            townList: [],
            relation_with_boss_house: [
                { value: '1', label: 'Es el jefe/a de hogar' },
                { value: '2', label: 'Esposo/a, compañero/a, conviviente del/la jefe/a de hogar' },
                { value: '3', label: 'Hijo/a, Hijastro/a del/la jefe/a de hogar' },
                { value: '4', label: 'Yerno/nuera del/la jefe/a de hogar' },
                { value: '5', label: 'Padres/suegros del/la jefe/a de hogar' },
                { value: '6', label: 'Hermano/a del/la jefe/a de hogar' },
                { value: '7', label: 'Otros parientes del/la jefe/a de hogar' }
            ],
            o: {
                id: null,
                region: null,
