<template>
    <v-page header="Inventario" action="/admin/inventory/item">
        <v-table :selectable="true" row-style-class="row.synchronized?'green':(row.tmpId>0?'yellow':'')"
            store="inventory" :scrollable="true" rowKey="id" :pagination="20" :filters="filters"
            src="/api/inventory/item">
            ...
            <template v-slot="{ row }">
                ...
                <td width="180" header="Código Patrimonial" class="center">
                    <v-filter>
                        <input v-model="filters.codigo_patrimonial" />
                    </v-filter>
                    {{ row.codigo_patrimonial }}
                </td>
                <td width="180" header="Código Inventario" class="center">
                    <v-filter>
                        <input v-model="filters.codigo_inventario" />
                    </v-filter>
                    {{ row.codigo_inventario }}
                </td>
                <td width="140" header="Tipo de Equipo / Accesorio">
                    <v-filter>
                ...
                <td width="160" header="Almacenamiento Capacidad">
                    <v-filter>
                        <input v-model="filters.almacenamiento_capacidad" />
                    </v-filter>
                    {{ row.almacenamiento_capacidad }}
                </td>
                <td width="160" header="Almacenamiento Tipo" class="center">
                    <v-filter>
                ...
                <td width="120" header="S.O. Versión">
                    <v-filter>
...
export default ui({
    data() {
        return { rowSelectedCount2: 0, page: 0, data: [], mode: 0, query: null, op: {} }
    },
    created() {
        const me = this;
